...
fit <- sampling(object = model,
                ...
                chains = 4,
                iter = 10000,
                sample_file = 'stan_samples/LCC9_vehicle.csv',
                diagnostic_file = 'stan_diagnostics/LCC9_vehicle.csv',
                seed = 12345,
                control = list(max_treedepth = 15))

trace <- traceplot(fit, pars = c('theta', 'sigma', 'y0_hat')) 
ggsave('results/images/traceplot/LCC9_vehicle_traceplot.jpeg', plot = trace)
...
write_csv(t, 'results/fitted_draws/LCC9_vehicle.csv')
...
ggsave('results/images/ppd/LCC9_vehicle_fit.jpeg', dpi = 300)

# Print summary stats
print(fit, pars = c("theta", "sigma", 'y0_hat'))

# Save summary stats
write_csv(summary(fit)$summary %>% as_tibble(rownames = 'par'), 'results/parameters/summary_statistics/LCC9_vehicle.csv')

# Save raw parameter ppd draws
fit %>% 
  spread_draws(theta[i]) %>% 
  write_csv('results/parameters/LCC9_vehicle.csv')
