#This file fits a bayesian logistic regression model
...
df <- read_csv("data/red_count_clean.csv")
#########
# Process data

df <- df %>% 
  select(well, field, rank, count, date) %>% 
  filter(well == 'B3' | well == 'B4') %>% 
  mutate(rank = rank(date)) %>%  # need to recalc ranks here in this specific case bc the section of data
                                 # was provided later and messed up the ranks in the cleaned dataset
  group_by(well, rank) %>% 
...
model <- stan_model(file = stan_file_path,
                    model_name = 'monoculture_logistic_growth_impute')
##########
# Fit the model

# specify parameters of interest
pars <- c("theta", "sigma", 'y_rep', 'y0_hat')
...
fit <- sampling(object = model,
                ...
                chains = 4,
                iter = 10000,
                sample_file = 'stan_samples/LCC9_treatment.csv',
                diagnostic_file = 'stan_diagnostics/LCC9_treatment.csv',
                seed = 123456)

trace <- traceplot(fit, pars = c('theta', 'sigma', 'y0_hat')) 
ggsave('results/images/traceplot/LCC9_treatment_traceplot.jpeg', plot = trace)
...
write_csv(t, 'results/fitted_draws/LCC9_treatment.csv')



ggplot(t, aes(x = rank, y = y_rep))+
  geom_line(aes(x = rank, y = y_rep_mean))+
  geom_ribbon(aes(ymin = .lower, ymax = .upper, group = .width), alpha = .15)+
  geom_point(aes(rank, agg_count))+
  theme_bw()+
  labs(x = 'Sampling time point', y = 'Cells per well')
ggsave('results/images/ppd/LCC9_treatment_fit.jpeg', dpi = 300)
...
# Save summary stats
write_csv(summary(fit)$summary %>% as_tibble(rownames = 'par'), 'results/parameters/summary_statistics/LCC9_treatment.csv')

# save raw parameter ppd draws
fit %>% 
  spread_draws(theta[i]) %>% 
  write_csv('results/parameters/LCC9_treatment.csv')
