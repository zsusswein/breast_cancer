...
fit <- sampling(object = model,
                  ...
                  chains = 4,
                  iter = 10000,
                  sample_file = 'stan_samples/LCC1_treatment.csv',
                  diagnostic_file = 'stan_diagnostics/LCC1_treatment.csv',
                  seed = 123456)  

trace <- traceplot(fit, pars = c('theta', 'sigma', 'y0_hat')) 
ggsave('results/images/traceplot/LCC1_treatment.jpeg', plot = trace)
...
write_csv(t, 'results/fitted_draws/LCC1_treatment.csv')
...
ggsave('results/images/ppd/LCC1_treatment_fit.jpeg', dpi = 300)

# Print summary stats
print(fit, pars = c("theta", "sigma", 'y0_hat'))

# Save summary stats
write_csv(summary(fit)$summary %>% as_tibble(rownames = 'par'), 'results/parameters/summary_statistics/LCC1_treatment.csv')

# Save raw parameter ppd draws
fit %>% 
  spread_draws(theta[i]) %>% 
  write_csv('results/parameters/LCC1_treatment.csv')
